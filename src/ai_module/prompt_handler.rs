...
pub fn load_prompt_file(filename: &str, prompt_dirs: &[String]) -> Result<String, AppError> {
    ...
    Err(AppError::Generic(format!("找不到提示文件: {}", filename)))
}
...
pub async fn send_prompt_and_get_response(
    ...
    let response = client
        ...
        .header("Authorization", format!("Bearer {}", config.ai.api_key.clone().unwrap_or_default()))
        .json(&request_data)
        .send()
        .await
        .map_err(|e| {
            AppError::Generic(format!("HTTP请求失败: {}", e))
        })?;
    ...
    if !response.status().is_success() {
        ...
        return Err(AppError::Generic(format!("API响应错误: {} - {}", status, error_text)));
    }

    // 解析JSON响应
    let response_json: Value = response.json().await.map_err(|e| {
        AppError::Generic(format!("JSON解析失败: {}", e))
    })?;
    ...
    let ai_response = response_json
        ...
        .ok_or_else(|| {
            AppError::Generic("无法从响应中提取AI生成的内容".to_string())
        })?
        .to_string();
...
/// 获取当前系统中可用的提示目录列表
pub fn get_prompt_directories(_config: &AppConfig) -> Vec<String> {
