...
async fn execute_git_command_with_error_handling(
    ...
    if let Ok(available) = is_git_available() {
        ...
        return Err(AppError::Generic("Failed to check if git is available".to_string()));
    }
    ...
    if let Ok(is_repo) = is_in_git_repository() {
        ...
        return Err(AppError::Generic("Failed to check if directory is a git repository".to_string()));
    }
        ...
        Err(app_error) => {
            ...
            if let AppError::Git(git_error) = &app_error {
                ...
                if should_use_ai(original_args) {
                    ...
                    if let GitError::CommandFailed { command, stderr, .. } = git_error {
                        // 尝试获取AI错误解释
